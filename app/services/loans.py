...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    loans = db.scalars(select(Loan).where(Loan.member_id == member.id)).all()
    if any(loan_status(loan, now) == "overdue" for loan in loans):
        raise HTTPException(status_code=409, detail="Member has an overdue loan")

    if any(loan.returned_at is None and loan.book_id == book.id for loan in loans):
        raise HTTPException(status_code=409, detail="Member already has this book on loan")

    limit = TIER_LOAN_LIMIT[member.tier]
    active_count = sum(loan.returned_at is None for loan in loans)
