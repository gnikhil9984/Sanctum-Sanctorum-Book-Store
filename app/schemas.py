...
def normalize_isbn13(raw: str) -> str:
    ...
    total = sum(int(digit) * (1 if index % 2 == 0 else 3) for index, digit in enumerate(isbn[:12]))
...
class OrderCreate(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_unique_books(self) -> OrderCreate:
        book_ids = [item.book_id for item in self.items]
        if len(book_ids) != len(set(book_ids)):
            raise ValueError("duplicate book_id values are not allowed")
        return self
